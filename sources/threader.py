...
def semAnalysis(self):
	...
	if isinstance(self.children[0], ListElementNode):
		...
		if not stack.hasVariable(token):
			print("error: uninitialized variable '%s'" %(token))
			AST.Node.nbSemErrors += 1
		elif trueType[0:4] != 'List':
			print("error: '%s' is not a list" %(token))
			AST.Node.nbSemErrors += 1
		elif type != trueType[5:] and type != 'unknown':
			print("error: Trying to assign type %s to a list element of type %s" %(type, trueType[5:]))
			AST.Node.nbSemErrors += 1
...
def semAnalysis(self):
	if not AST.Node.scopeStack.mergeTypes():
		print("error: operation members have incompatible types")
		AST.Node.nbSemErrors += 1
	self.next[0].semAnalysis()
...
def semAnalysis(self):
	...
	if listType == 'Forbidden':
		print("error: all list elements must be the same type")
		AST.Node.nbSemErrors += 1
...
def semAnalysis(self):
	...
	if type != 'Integer' and \
	   type != 'unknown' :
		print("error: wrong type for index")
		AST.Node.nbSemErrors += 1
...
def semAnalysis(self):
	...
	if type != 'Double' and \
	   ...
	   type != 'Boolean' and \
	   type != 'unknown' :
		print("error: wrong type for condition")
		AST.Node.nbSemErrors += 1
...
def semAnalysis(self):
	if AST.Node.scopeStack.hasFunction(self.children[0].tok):
		...
		print("error: function '%s' already defined" %(self.children[0].tok))
		AST.Node.nbSemErrors += 1
	else:
...
def semAnalysis(self):
	...
	if type1 != 'Integer' and type1 != 'unknown':
		print("error: range first type should be integer")
		AST.Node.nbSemErrors += 1
	if type2 != 'Integer' and type2 != 'unknown':
		print("error: range second type should be integer")
		AST.Node.nbSemErrors += 1
	self.next[0].semAnalysis()
...
def semAnalysis(self):
	...
	if type[0:4] != 'List' and type != 'unknown':
		print("error: %s is not a list" %(self.children[1].tok))
		AST.Node.nbSemErrors += 1
	self.next[0].semAnalysis()
